...
with st.sidebar:
    st.header("Ключи источников")
    st.caption(
        "Без ключей сайт работает и показывает оценки. С ключами те же поля "
        "заполняются реальной статистикой площадок."
    )
    with st.expander("Яндекс.Вордстат", expanded=True):
...
st.markdown(
    ...
    <div class="hero">
      ...
      <p>Один запрос — сразу Яндекс, Wildberries, Ozon и Google. Например:
         «ароматизатор в машину», «магнитный держатель для телефона».</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.expander("Откуда берутся цифры"):
    st.markdown(
        """
- **Яндекс** — Wordstat API (или шлюз XMLRiver): реальное число показов фразы за месяц.
- **Wildberries** — отчёт «Поисковые запросы» из аналитики продавца, если задан токен.
  Без токена — оценка по числу товаров в публичной выдаче WB.
- **Ozon** — статистика поисковых фраз Performance API, если заданы Client Id и Secret.
  Без них — оценка по публичной выдаче Ozon.
- **Google** — оценка от частотности Вордстата по доле Google в поиске РФ.

Оценки помечены жёлтым бейджем: они годятся для сравнения запросов между собой,
но не заменяют официальную статистику. Коэффициенты модели лежат в
`frequency/estimate.py` — их можно откалибровать под свою нишу.
        """
    )
